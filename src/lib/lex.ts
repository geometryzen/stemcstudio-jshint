...
type TriggerType = 'error' | 'fatal' | 'warning' | 'Identifier' | 'NoSubstTemplate' | 'Number' | 'String' | 'TemplateHead' | 'TemplateMiddle' | 'TemplateTail';
...
export class Lexer {
    ...
    private pushContext(ctxType: number) {
        this.context.push({ type: ctxType });
    }

    private popContext(): ILexerContext {
    ...
    */
    ...
    private currentContext(): ILexerContext {
     ...
     */
    private trigger(type: TriggerType, context: { code?: string; line: number; char?: number; character?: number; startLine?: number; base?: number; isMalformed?: boolean; startChar?: number; from?: number; data?: string[]; value?: string; }) {
    ...
    scanRegExp(): IToken {
        ...
        var scanUnexpectedChars = function (this: Lexer) {
