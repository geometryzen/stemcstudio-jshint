...
// JavaScript URL (jx)
var javascriptURL = /^(?:javascript|jscript|ecmascript|vbscript|livescript)\s*:/i;
// Catches /* falls through */ comments (ft)
var fallsThrough = /^\s*falls?\sthrough\s*$/;
...
var nonAsciiIdentifierStartTable = str.split(',').map(function (code) {
...
var nonAsciiIdentifierPartTable = str$1.split(',').map(function (code) {
    return parseInt(code, 10);
});
...
var validNames = Object.keys(val)
...
function removeIgnoredMessages() {
    ...
    var errors$$1 = JSHINT.errors;
    JSHINT.errors = reject(errors$$1, function excluder(err) { return ignored[err.line]; });
}
/**
    ...
    if (nt.type === "jshint" || nt.type === "jslint") {
        body.forEach(function (g) {
            ...
            if (numvals.indexOf(key) >= 0) {
                ...
                if (val$$1 !== "false") {
                    val$$1 = +val$$1;
                    if (typeof val$$1 !== "number" || !isFinite(val$$1) || val$$1 <= 0 || Math.floor(val$$1) !== val$$1) {
                        error("E032", nt, g[1].trim());
                        return;
                    }
                    state.option[key] = val$$1;
                }
                else {
             ...
             */
            if (key === "es5") {
                if (val$$1 === "true" && state.option.es5) {
                    warning("I003");
                }
            }
            if (key === "validthis") {
                ...
                state.option.validthis = (val$$1 === "true");
                return;
            }
            if (key === "quotmark") {
                switch (val$$1) {
                    case "true":
                    case "false":
                        state.option.quotmark = (val$$1 === "true");
                        break;
                    case "double":
                    case "single":
                        state.option.quotmark = val$$1;
                        break;
                    default:
                        error("E002", nt);
                }
            ...
            if (val$$1 === "true" || val$$1 === "false") {
                if (nt.type === "jslint") {
                    tn = renamed[key] || key;
                    state.option[tn] = (val$$1 === "true");
                    if (inverted[tn] !== void 0) {
                        state.option[tn] = !state.option[tn];
                    }
                }
                else {
                    state.option[key] = (val$$1 === "true");
                }
                return;
            }
            error("E002", nt);
...
function isPoorRelation(node$$1) {
    return node$$1 &&
        ((node$$1.type === "(number)" && +node$$1.value === 0) ||
            (node$$1.type === "(string)" && node$$1.value === "") ||
            (node$$1.type === "null" && !state.option.eqnull) ||
            node$$1.type === "true" ||
            node$$1.type === "false" ||
            node$$1.type === "undefined");
}
var typeofValues = {};
typeofValues.legacy = [
...
function isTypoTypeof(left, right, state$$1) {
    var values;
    if (state$$1.option.notypeof)
        return false;
    if (!left || !right)
        return false;
    values = state$$1.inES6() ? typeofValues.es6 : typeofValues.es3;
    if (right.type === "(identifier)" && right.value === "typeof" && left.type === "(string)")
        return !contains(values, left.value);
    return false;
}
function isGlobalEval(left, state$$1) {
    var isGlobal = false;
    // permit methods to refer to an "eval" key in their own context
    if (left.type === "this" && state$$1.funct["(context)"] === null) {
        isGlobal = true;
    }
    else if (left.type === "(identifier)") {
        if (state$$1.option.node && left.value === "global") {
            isGlobal = true;
        }
        else if (state$$1.option.browser && (left.value === "window" || left.value === "document")) {
            isGlobal = true;
        }
    }
    return isGlobal;
...
function optionalidentifier(fnparam, prop, preserve) {
    ...
    var curr = state.tokens.curr;
    var val$$1 = state.tokens.curr.value;
    if (!isReserved(curr)) {
        return val$$1;
    }
    if (prop) {
...
function createMetrics(functionStartToken) {
    return {
        ...
        verifyMaxParametersPerFunction: function () {
            if (isNumber$1(state.option.maxparams) &&
...
function destructuringPatternMatch(tokens, value) {
    ...
    zip(tokens, Array.isArray(first) ? first : [first]).forEach(function (val$$1) {
        var token = val$$1[0];
        var value = val$$1[1];
        if (token && value) {
            token.first = value;
 ...
 */
var JSHINT = function lint(s, o, g) {
    o = clone(o);
    state.reset();
    if (o && o.scope) {
    ...
    if (o) {
        ...
        each$1(o.exported || null, function (value, item) {
            exported[item] = true;
        });
        delete o.predef;
        delete o.exported;
